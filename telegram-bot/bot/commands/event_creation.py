...
async def start_event_flow(
    ...
    if mode == "public" and chat_type not in {"group", "supergroup"}:
        await message.reply_text("ℹ️ This command can only be used in a Telegram group.")
        return
    ...
    if not settings.db_url:
        await message.reply_text("ℹ️ Database configuration is unavailable.")
        return
    ...
    if context.user_data is None:
        await message.reply_text("ℹ️ User session data is unavailable.")
        return
...
async def _handle_callback_common(
    ...
    if data and data.startswith(f"{prefix}_edit_"):
        ...
        if target in edit_order:
            for downstream_key in [
                ...
                if edit_order.index(target) < edit_order.index(
                ...
                elif target == "date_preset" and downstream_key in [
                    "scheduled_date",
                    "scheduled_time",
                    "time_window",
                    "min_participants",
                    ...
                    flow_data.pop(downstream_key, None)
                elif target == "time_window" and downstream_key in [
                    "min_participants",
                    "target_participants",
                    "duration_minutes",
                    "location_type",
                    ...
                    flow_data.pop(downstream_key, None)
                elif target == "threshold" and downstream_key in [
                    "min_participants",
                    "target_participants",
                    "duration_minutes",
                    "location_type",
                    ...
                    flow_data.pop(downstream_key, None)
                elif target == "duration" and downstream_key in [
                    "duration_minutes",
                    "location_type",
                    "budget_level",
                    "transport_mode",
                    ...
                    flow_data.pop(downstream_key, None)
                elif target == "location" and downstream_key in [
                    "location_type",
                    "budget_level",
                    "transport_mode",
                    "invitee_mode",
        ...
        if target == "description":
            ...
            await _safe_edit_message(
                ...
                reply_markup=build_threshold_markup(back_target, prefix=prefix),
            )
        ...
        if preset == "custom":
            ...
            if not choices:
                await _safe_edit_message(
                    query,
                    "ℹ️ Could not resolve that date preset. Please try again.",
                    reply_markup=build_date_preset_markup(prefix=prefix),
                )
                return
            if len(choices) == 1:
        ...
        try:
            ...
            await _safe_edit_message(query, "ℹ️ Invalid date option selected.")
            return
        ...
        if window not in TIME_WINDOWS:
            await _safe_edit_message(query, "ℹ️ Unsupported time window.")
            return
        ...
        if len(option) != 4 or not option.isdigit():
            await _safe_edit_message(query, "ℹ️ Invalid time option.")
            return
        hour = int(option[:2])
        minute = int(option[2:])
        ...
        if not isinstance(scheduled_date, str):
            await _safe_edit_message(
                query,
                "ℹ️ Event date is missing. Please pick date again.",
                reply_markup=build_date_preset_markup(prefix=prefix),
            )
            return
        try:
            ...
            await _safe_edit_message(query, "ℹ️ Could not parse selected time.")
            return
    ...
    # v3.2: Handle target_participants selection
        ...
        await _safe_edit_message(query, "ℹ️ Event creation stepped back.")
...
async def _handle_message_common(
    ...
    if stage == "description":
        ...
        try:
            ...
            prefix = "private_event" if mode == "private" else "event"
            await update.message.reply_text(
                "ℹ️ Invalid handle list. Use comma-separated @handles.\n"
                "Example: @alice, @bob_builder\n"
                "Or use: @all",
                reply_markup=build_compact_markup(
                    [],
        ...
        if not text:
            await update.message.reply_text(
                "ℹ️ Send text modifications, or press Confirm/Discard.",
                reply_markup=build_final_confirmation_markup(prefix=prefix),
            )
            return
...
async def finalize_event(
    ...
    if not isinstance(event_flow_raw, dict):
        await _safe_edit_message(query, "ℹ️ Event flow not found.")
        return

    event_flow: dict[str, Any] = event_flow_raw
    data_raw = event_flow.get("data")
    if not isinstance(data_raw, dict):
        await _safe_edit_message(query, "ℹ️ Event flow data is invalid.")
        return
    ...
    if scheduling_mode != "flexible" and not isinstance(scheduled_time_raw, str):
        await _safe_edit_message(query, "ℹ️ Event time is missing.")
        return

    group_id = event_flow.get("group_id")
    if not isinstance(group_id, int):
        await _safe_edit_message(query, "ℹ️ Group context is missing.")
        return
    ...
    async with get_session(settings.db_url) as session:
        ...
        if conflict:
            await _safe_edit_message(
                query,
                "ℹ️ Creator has a conflicting event.\n"
    ...
    logger.info(
        "[INVITATION_FLOW] Event created | event_id=%s group_id=%s invite_all=%s invitee_count=%d",
        event.event_id,
        group_id,
        data.get("invite_all_members", False),
        len(invitees),
    )
    ...
    async with get_session(settings.db_url) as session:
        ...
        # Bug 1+4+7: Fetch real Telegram members when invite_all is True, deduplicate recipients
        invite_all = bool(data.get("invite_all_members"))
        explicit_invitees = list(data.get("invitees", []))

        # Filter out the "@all" sentinel value from explicit invitees
        explicit_invitees = [h for h in explicit_invitees if h != "@all"]

        # Build deduplicated recipient set (set prevents duplicates)
        recipient_telegram_ids: set[int] = set()

        logger.info(
            "[INVITATION_FLOW] Building recipient list | event_id=%s invite_all=%s has_group=%s telegram_group_id=%s",
            event.event_id,
            invite_all,
            group is not None,
            group.telegram_group_id if group else None,
        )
        ...
        explicit_tg_ids = set()
        logger.info(
            "[INVITATION_FLOW] Processing explicit invitees | event_id=%s explicit_count=%d",
            event.event_id,
            len(explicit_invitees),
        )
        for handle in explicit_invitees:
            if not handle.startswith("@"):
                logger.warning(
                    "[INVITATION_FLOW] Invalid handle format (missing @) | event_id=%s handle=%s",
                    event.event_id,
                    handle,
                )
                continue
            ...
            try:
                ...
                if user_id:
                    ...
                    if invitee_user and invitee_user.telegram_user_id:
                        ...
                        explicit_tg_ids.add(tg_id)
                        recipient_telegram_ids.add(tg_id)
                        logger.info(
                            "[INVITATION_FLOW] Resolved explicit invitee | event_id=%s username=%s telegram_id=%s",
                            event.event_id,
                            username,
                            tg_id,
                        )
                        ...
                        logger.warning(
                            "[INVITATION_FLOW] User found but no telegram_id | event_id=%s username=%s user_id=%s",
                            event.event_id,
                            username,
                            user_id,
                        )
                    ...
                    logger.warning(
                        "[INVITATION_FLOW] Username not found in DB | event_id=%s username=%s",
                        event.event_id,
                        username,
                    )
            except Exception as e:
                logger.error(
                    "[INVITATION_FLOW] Failed to resolve invitee | event_id=%s username=%s error=%s",
                    event.event_id,
                    username,
                    str(e),
                    exc_info=True,
        ...
        if explicit_tg_ids:
            logger.info(
                "[INVITATION_FLOW] Explicit invitees added | event_id=%s count=%d",
                event.event_id,
                len(explicit_tg_ids),
            )

        # Exclude creator from recipient list (they get a separate admin DM)
        recipient_telegram_ids.discard(creator_id)

        logger.info(
            "[INVITATION_FLOW] Recipients finalized | event_id=%s total_recipients=%d "
            "cached_members=%d explicit_invitees=%d invite_all=%s creator_excluded=%s",
            event.event_id,
            len(recipient_telegram_ids),
        ...
        if not recipient_telegram_ids:
            logger.error(
                "[INVITATION_FLOW] NO RECIPIENTS - invitations cannot be sent | event_id=%s. "
                "Check: 1) member_list empty, 2) explicit invitees invalid, 3) invite_all=False",
                event.event_id,
            )

        # Send DMs with rate limiting (Bug 6: prevent Telegram API rate limit hits)
        dm_count = 0
        dm_failed = 0
        dm_skipped_no_bot = 0
        RATE_LIMIT_DELAY = (
        ...
        if not context.bot:
            logger.error(
                "[INVITATION_FLOW] Cannot send DMs - context.bot is None | event_id=%s",
                event.event_id,
            )
            dm_skipped_no_bot = len(recipient_telegram_ids)
        else:
            logger.info(
                "[INVITATION_FLOW] Starting DM distribution | event_id=%s recipient_count=%d",
                event.event_id,
                len(recipient_telegram_ids),
            )
            for telegram_user_id in recipient_telegram_ids:
                try:
                    logger.info(
                        "[INVITATION_FLOW] Sending invitation DM | event_id=%s recipient=%s",
                        event.event_id,
                        telegram_user_id,
                    )
                    ...
                    if sent:
                        dm_count += 1
                        logger.info(
                            "[INVITATION_FLOW] DM sent successfully | event_id=%s recipient=%s",
                            event.event_id,
                            telegram_user_id,
                        )
                    else:
                        dm_failed += 1
                        logger.warning(
                            "[INVITATION_FLOW] DM failed (send_event_invitation_dm returned False) | "
                            f"event_id={event.event_id} recipient={telegram_user_id}",
                        )
                except Exception as e:
                    dm_failed += 1
                    logger.error(
                        "[INVITATION_FLOW] DM send exception | event_id=%s recipient=%s error_type=%s error=%s",
                        event.event_id,
                        telegram_user_id,
                        type(e).__name__,
                        str(e),
        ...
        logger.info(
            "[INVITATION_FLOW] Distribution complete | event_id=%s sent=%d failed=%d "
            "skipped_no_bot=%d total_expected=%d",
            event.event_id,
            dm_count,
            dm_failed,
            dm_skipped_no_bot,
        ...
        # Post live card to group chat
        if group and group.telegram_group_id:
            try:
                from telegram import InlineKeyboardButton, InlineKeyboardMarkup
                from bot.services import LiveCardService

                type_emoji = {
                    "sports": "🏃",
                    "social": "🍕",
                    "work": "💻",
                }.get(data.get("event_type", "general"), "🎯")

                card_text = (
                    f"{type_emoji} *New {data.get('event_type', 'general').capitalize()} Event*\n\n"
                    f"*{data.get('description', 'No description')}*\n\n"
                    f"📅 {format_scheduled_time(data.get('scheduled_time'))}\n"
                    f"👤 {organizer_display_name or organizer_username or 'Unknown'}\n"
                    f"👥 1 / {data.get('min_participants', 2)} needed"
                )
                ...
                live_card_service = LiveCardService(session, context.bot)
                await live_card_service.create_live_card(
                    event_id=event.event_id,
...
async def finalize_private_event(
    ...
    await _safe_edit_message(
        ...
        f"Capacity: {_escape_md(data.get('target_participants', 'Not set'))}\n"
        f"Invitees: {_escape_md(invitees_summary)}\n\n"
        f"📋 Awaiting confirmations from invitees.\n"
        f"Run /confirm {event.event_id} to lock when ready."
        + (
            f"\n\nEvent Admin: {organizer_username if organizer_username else creator_id}"
            if organizer_username
            else f"\n\nEvent Admin: {creator_id}"
        ),
